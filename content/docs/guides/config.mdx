...
<Callout type="warn">
**Container mode: `KODO_MASUMI*` env vars may be invisible at startup**

In container deployments, indexed Masumi environment variables (`KODO_MASUMI1`, `KODO_MASUMI2`, etc.) are not reliably visible to `_parse_masumi_env()` at startup. The parser may silently skip them, leaving your Masumi configuration unconfigured with no error message. This is a [known bug](https://github.com/masumi-network/kodosumi/issues/59).

**Workaround:** Until a fix is released, either:
- Mount a `.env` file into the container and let Kodosumi load it at startup, or
- Use the non-indexed `KODO_MASUMI_<KEY>=<value>` form if your configuration supports it
</Callout>
