...
<Callout type="warn">
**Thundering herd: starting multiple Serve apps simultaneously can cause OOM crashes**

When several Ray Serve apps initialise at the same time — for example after a node restart or a bulk `koco deploy` — they may all request objects from the Ray Global Control Service (GCS) concurrently. This saturates the GCS object store and can trigger OOM crashes on memory-constrained nodes. This is a [known issue](https://github.com/masumi-network/kodosumi/issues/60).

**Mitigations:**
- Deploy apps **sequentially** rather than bundling them all into one `koco deploy` call
- Start with `num_replicas: 1` per app and scale up only after the cluster has stabilised
- Reduce per-replica `num_cpus` to limit concurrent initialisation pressure
- See Ray's guide on [limiting concurrency to avoid OOM](https://docs.ray.io/en/latest/ray-core/patterns/limit-running-tasks.html#pattern-using-resources-to-limit-the-number-of-concurrently-running-tasks)
</Callout>

Be advised to gather some experience with Ray core components before you rollout your services. Understand [remote resource requirements](https://docs.ray.io/en/latest/ray-core/scheduling/resources.html#resource-requirements) and how to [limit concurrency to avoid OOM issues](https://docs.ray.io/en/latest/ray-core/patterns/limit-running-tasks.html#pattern-using-resources-to-limit-the-number-of-concurrently-running-tasks)
