...
<Callout type="warn" title="Breaking change: --register removed from koco serve and koco start">
The `--register` option has been removed from `koco serve` (and `koco start`) in recent versions.
Passing it now causes an immediate crash on startup:

```
Error: No such option: --register
```

Services that crash on startup will enter a `systemd` restart loop. If your service files,
Docker `CMD` entries, or startup scripts include `--register`, **remove it before upgrading**.

See [kodosumi#57](https://github.com/masumi-network/kodosumi/issues/57) for background.
</Callout>
...
- ~~`--register TEXT`~~ — **Removed.** This option no longer exists. Passing it crashes the process. Remove it from any service files or startup scripts. See [kodosumi#57](https://github.com/masumi-network/kodosumi/issues/57).
...
- `--exec-dir TEXT` - Execution directory
- ~~`--register TEXT`~~ — **Removed.** See warning above.
