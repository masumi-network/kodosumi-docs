...
[![Hymn](/assets/panel/thumb/form.png)](/assets/panel/form.png)
...
<Callout type="warn">
**Known issue — Sumi status endpoint may expose full Python tracebacks** ([kodosumi#55](https://github.com/masumi-network/kodosumi/issues/55))

The panel API (`GET /outputs/status/{fid}`) behaves as described above. However, the **Sumi-facing endpoint** (`GET /sumi/{expose}/{meta}/status/{job_id}`) used by Sokosumi can return a full Python traceback in the `error` field when an exception other than `KodosumiError` propagates unhandled. In environments where Ray module paths include access tokens (e.g. private GitHub pip installs), **this exposes credentials to anyone who can query the status endpoint.**

Until this is patched, protect your agents with three steps:

1. **Wrap all agent code in try/except** and re-raise unexpected exceptions as `KodosumiError` with a user-safe message:
   ```python
   from kodosumi.error import KodosumiError
   try:
       result = do_something_risky()
   except KodosumiError:
       raise  # already safe — pass through
   except Exception as e:
       raise KodosumiError("Something went wrong. Please try again.") from e
   ```
2. **Never render the raw `error` field to end users** without sanitizing it — treat it as untrusted internal output.
3. **Rotate any tokens** that may have appeared in error responses from production Sumi endpoints.
</Callout>
